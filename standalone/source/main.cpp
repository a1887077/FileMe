...
int main(int argc, char** argv) {
  ...
  if (argc == 2) {
    fs::path workspace = std::string(argv[1]);

    int ret = interface.nav_to_dir(workspace);

    if (ret == FileOperator::SUCCESS) {
      interface.update_file_list();
    } else {
      interface.show_error("Please provide a valid path");
      std::this_thread::sleep_for(std::chrono::milliseconds(750));
      return -1;
    }
  } else if (argc > 2) {
    interface.show_error("Invalid number of arguments");
    std::this_thread::sleep_for(std::chrono::milliseconds(750));
    return -1;
  }
  ...
  while (quit == false) {
    ...
    switch (command) {
      ...
      case KEY_UP:
        ...
        interface.update_file_list(); // update the display interface
        break;
      ...
      case KEY_BACKSPACE:
        ...
        interface.update_file_list(); // update the interface display
        interface.show_path(); // show current path
        break;
      ...
      case 'P':
        if(interface.paste() == FileOperator::SUCCESS){ // if item is pasted correctly
          ...
          interface.update_file_list(); // update display
          interface.show_path(); // show current path
